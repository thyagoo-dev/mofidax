...
import { UploadCloud, FileType2, Download, Loader2, X, FileImage, CheckSquare, ImagePlus } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { PDFDocument } from 'pdf-lib';
import JSZip from 'jszip';
import { useHistoryStore } from '../store/useHistoryStore';

// Importando o motor de renderização da Mozilla
...
export function PdfToImagePage() {
  ...
  const renderThumbnails = async (buffer: ArrayBuffer, numPages: number) => {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        ...
        const viewport = page.getViewport({ scale: 0.3 }); // Baixa resolução para UI
  ...
  // O motor de extração em alta resolução atualizado
  const handleExtractImages = async () => {
    if (!pdfFile || !pdfBuffer || selectedPages.size === 0) return;
    ...
    try {
      ...
      // CASO 1: Apenas uma imagem -> Baixar direto (sem ZIP)
      if (pagesToExtract.length === 1) {
        ...
        if (context) {
          ...
          if (blob) {
            ...
            const fileName = `${baseName}_pg${pageNum}.${ext}`;
            
            // Simula o clique de download
      ...
      // CASO 2: Múltiplas imagens -> Usar JSZip
      else {
        const zip = new JSZip();
        for (const pageNum of pagesToExtract) {
          ...
          if (context) {
            ...
            if (blob) zip.file(`${baseName}_pg${pageNum}.${ext}`, blob);
          }
        }

        const zipBlob = await zip.generateAsync({ type: 'blob' });
        const url = URL.createObjectURL(zipBlob);
        const zipFileName = `mofidax_imagens_${baseName}.zip`;

        setZipUrl(url);
        setStats({ size: zipBlob.size });
      ...
      <div className="mb-6 sm:mb-8 text-center w-full">
        <h1 className="text-2xl sm:text-3xl font-bold mb-3 tracking-tight flex flex-row items-center justify-center gap-3">
          <div className="p-2 bg-rose-400/10 rounded-xl border border-rose-400/20">
            <FileImage className="w-5 h-5 sm:w-6 sm:h-6 text-rose-400" />
          </div>
          PDF para Imagem
        </h1>
        <p className="text-sm sm:text-base text-secondary max-w-[280px] sm:max-w-md mx-auto leading-relaxed">
          Converta páginas do seu PDF em imagens JPG ou PNG de alta qualidade num piscar de olhos.
        </p>
      </div>
        ...
        {!pdfFile ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-black/20 p-5 rounded-xl border border-white/5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                <label className="text-sm font-medium flex items-center gap-2">
                  ...
                  Páginas para converter ({selectedPages.size})
                </label>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex bg-black/30 rounded-lg p-1 border border-white/5">
                    <button
                      onClick={() => { setFormat('jpeg'); setZipUrl(null); }}
                      className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${format === 'jpeg' ? 'bg-rose-500 text-white shadow-sm' : 'text-secondary hover:text-primary'}`}
                    >
                      JPG
                    </button>
                    <button
                      onClick={() => { setFormat('png'); setZipUrl(null); }}
                      className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${format === 'png' ? 'bg-rose-500 text-white shadow-sm' : 'text-secondary hover:text-primary'}`}
                    >
                      PNG
                    </button>
                  </div>
                  ...
                  <div className="flex gap-2 text-xs border-l border-white/10 pl-4">
                    <button onClick={selectAll} className="text-secondary hover:text-rose-400 transition-colors">Tudo</button>
                    <button onClick={clearSelection} className="text-secondary hover:text-rose-400 transition-colors">Limpar</button>
                  </div>
                </div>
              </div>
            ...
            {!zipUrl ? (
              ...
              >
                {isProcessing ? (
                  <><Loader2 className="w-5 h-5 animate-spin" /> Processando Imagens...</>
                ) : (
                  <><ImagePlus className="w-5 h-5" /> Extrair Imagens</>
                )}
              </button>
            ) : (
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between text-xs sm:text-sm bg-blue-500/10 border border-blue-500/20 p-3 rounded-xl text-blue-400">
                  <span>Pacote ZIP Gerado ({selectedPages.size} imagens)</span>
                  <span className="font-mono font-bold">{formatBytes(stats?.size || 0)}</span>
                </div>
                
                <a
                  href={zipUrl}
                  download={`mofidax_imagens_${pdfFile.name.replace(/\.[^/.]+$/, "")}.zip`}
                ...
                >
                  <Download className="w-5 h-5" /> Baixar Imagens (ZIP)
                </a>
              </div>
            )}
          </div>
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-white/10 bg-black/20">
                <h3 className="font-medium text-primary flex items-center gap-2">
                  <FileType2 className="w-5 h-5 text-rose-400" /> Pré-visualização
                </h3>
